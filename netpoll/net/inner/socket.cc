...
int Socket::createNonblockingSocketOrDie(int family)
{
	...
	if (sock < 0)
	{
		ELG_FATAL("sockets::createNonblockingOrDie");
	}
	ELG_TRACE("sock={}", sock);
	return sock;
}
...
void Socket::setNonBlockAndCloseOnExec(int sockfd)
{
	...
	if (ret)
	{ ELG_ERROR("Socket::fcntl set nonblock"); }
	// close-on-exec
	flags = ::fcntl(sockfd, F_GETFD, 0);
	flags |= FD_CLOEXEC;
	ret = ::fcntl(sockfd, F_SETFD, flags);
	if (ret)
	{ ELG_ERROR("Socket::fcntl set close-on-exec"); }
#endif
}
...
void Socket::bindAddress(const InetAddress& localaddr)
{
	...
	{
		ELG_FATAL("Bind address failed at {}",
			localaddr.toIpPort());
	}
}
void Socket::listen()
